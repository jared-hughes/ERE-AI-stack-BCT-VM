#include"raw-a.h"
typedef struct E{int t;union{struct{struct E*l,*r;}c;C*v;int r;struct{int f;struct E*a;}ap;};}E;
typedef struct{C*s;E*d;}O;E*me(int t)_(E*e=malloc(sizeof(E));e->t=t;e)
E*mc(E*l,E*r)_(E*e=me(0);e->c.l=l;e->c.r=r;e)E*mf(C*v)_(E*e=me(1);e->v=v;e)
E*mr(int r)_(E*e=me(2);e->r=r;e)E*ma(int f,E*a)_(E*e=me(3);e->ap.f=f;e->ap.a=a;e)
FILE*in;U l,h=0xffffffffffffffffLL,x;D cxt,_pr=0xFFFFFFFF;
int ct[512][2];V up(int y){IF(++ct[cxt][y]>0xFFFFFFF,ct[cxt][0]>>=1,ct[cxt][1]>>=1)
  cxt=(cxt<<1)|y;cxt&=0xff;_pr=(((U)(ct[cxt][1]+1)<<32)/(ct[cxt][0]+ct[cxt][1]+2));}
int rb(){U m=l+((h-l)>>32)*_pr;int y=x<=m;if(y)h=m;else l=m+1;up(y);
  W(((l^h)&0xff00000000000000LL)==0){l=l<<8;h=h<<8|0xff;int c=getc(in);if(c==EOF)c=0;x=x<<8|c;}R y;}
...
int pre(C*w, C*z){W(*w)IF(*w!=*z,R 0)ELSE(w++,z++)R 1;}C*eval(int f, C*in);
...
C*eval(int f,C*in){O*ops=fs[f];re:Fj(strlen(in),Fi(fns[f],if(pre(ops[i].s,in+j)){
  ...
  goto re;}))R in;}
int main(int ac, C*av[]){in=fopen(av[1],"rb");
    Fi(8,int c=getc(in);if(c==EOF)c=0;x=x<<8|c)pf();C*ni=malloc(strlen(av[2])+5);
